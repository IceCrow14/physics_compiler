...
function module.get_node_count(jms_path)
	local node_count
	local i = 1
	for line in io.lines(jms_path) do
...
function line_to_table(line) -- Takes a line (string) from a file and returns tab separated (numeric) values to a table
	local data = {}
	local i = 1
...
function module.get_node_data(jms_path)
	local node_data = {}
	local node_count = module.get_node_count(jms_path)
	local i = 1
	local skip
	...
	for line in io.lines(jms_path) do
		...
		while not skip do
			...
			elseif not node_relative_rotation_quaternion then
				node_relative_rotation_quaternion = line_to_table(line)
			...
			elseif not node_relative_translation then
				node_relative_translation = line_to_table(line)
...
function module.get_materials_count(jms_path) -- It is not necessary to extract materials data for a .physics tag, but it is to get the amount of materials and count the lines such section takes on the file
...
function module.get_mass_point_count(jms_path)
	local mass_point_count
	local node_count = module.get_node_count(jms_path)
	local materials_count = module.get_materials_count(jms_path)
	local i = 1
	local skip
...
function module.get_mass_point_data(jms_path)
	local mass_point_data = {}
	local node_count = module.get_node_count(jms_path)
	local materials_count = module.get_materials_count(jms_path)
	local mass_point_count = module.get_mass_point_count(jms_path)
	local i = 1
	local skip
	...
	local mass_point_first_child_index
	local mass_point_next_sibling_index
	...
	for line in io.lines(jms_path) do
		...
		while not skip do
			...
			elseif not mass_point_first_child_index then
				mass_point_first_child_index = tonumber(line)
				mass_point_data[mass_point_index].first_child_index = mass_point_first_child_index
			elseif not mass_point_next_sibling_index then
				mass_point_next_sibling_index = tonumber(line)
				mass_point_data[mass_point_index].next_sibling_index = mass_point_next_sibling_index
			elseif not mass_point_relative_rotation_quaternion then
				mass_point_relative_rotation_quaternion = line_to_table(line)
			...
			elseif not mass_point_relative_translation then
				mass_point_relative_translation = line_to_table(line)
			...
			elseif not mass_point_radius then
				...
				mass_point_name = nil
				mass_point_first_child_index = nil
				mass_point_next_sibling_index = nil
